...
class UpdatePermissionMixin(AccessMixin):
    ...
    def dispatch(self, request: HttpRequest, *args, **kwargs):
        ...
        user_related_model = user[0] if (user :=[
            related 
            for related in obj._meta.fields
            if isinstance(related, ForeignKey)
            and related.related_model ==  User
            ])  else None
        ...
        user_pk = obj.__class__.objects.filter(
            **{user_field_name: current_user}
            ).values_list(user_field_name+"__pk").first()
